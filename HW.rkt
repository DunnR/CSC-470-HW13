...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname HW) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ())))
;Global Variables
...
(define extend-env-4-let
  (lambda (app-exp env)
    (if (null? app-exp)
        env
        (if (not (eq? (car app-exp) 'app-exp))
            (extend-env-4-let (cdr app-exp) (extend-env-4-let (car app-exp) env))
            (extend-env-4-lambda (list (list-ref (list-ref app-exp 1) 1)) (list (eval-exp (list-ref app-exp 2) env)) env)))))
